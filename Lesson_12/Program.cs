...
class Program
{
    ...
    static void Main()
    {
        ...
        //    debtors
        //.Where(db => db.Email.Contains("rhyta.com") || db.Email.Contains("dayrep.com"))
        //.ToList()
        ...
        //debtors.FindAll(delegate (Debtor db)
        //{
        //    int age = DateTime.Now.Year - db.BirthDay.Year ;
        //    if (db.BirthDay.Date > DateTime.Today.AddYears(-age)) age--;
        //    if (age >= 26 && age <= 36) return true;
        //    else return false;
        ...
        //debtors.FindAll(db => db.Debt > 5000).ForEach(db => Console.WriteLine(db));

        #endregion

        #region  5) Butov adi 18 simvoldan cox olan ve telefon nomresinde 2 ve ya 2 - den cox 7 reqemi olan borclulari cixartmaq
        //debtors.FindAll(db => db.FullName.Length > 18)
        //    .FindAll(db => db.Phone.Count(s => s == '7') >= 2)
        //    .ForEach(db => Console.WriteLine(db));
        #endregion

        #region 7) Qishda anadan olan borclulari cixardmaq

        //debtors.FindAll(db => db.BirthDay.Month == 12 || db.BirthDay.Month == 1 || db.BirthDay.Month == 2)
        //    .ForEach(db => Console.WriteLine(db));
        ...
        //debtors.FindAll(db => db.Debt > (debtors.Sum(db => db.Debt) / debtors.Count))
        //    .Sort((db, db2) => db.FullName.Split(' ')[2].CompareTo(db2.FullName.Split(' ')[2]));
        //debtors.Sort((db, db2) => db.Debt.CompareTo(db2.Debt));

        #endregion

        #region 9) Telefon nomresinde 8 olmayan borclularin yalniz familyasin, yashin ve umumi borcun meblegin cixarmaq

        //debtors.FindAll(db => !db.Phone.Contains('8'))
        //    .ForEach(db => Console.WriteLine($"{db.FullName.Split(' ')[2]} { delegate (Debtor db) { int age = DateTime.Now.Year - db.BirthDay.Year; if (db.BirthDay.Date > DateTime.Today.AddYears(-age)) age--; Console.Write(age);}} {db.Debt}"));
        #endregion
        ...
        //    debtors
        //.FindAll(delegate (Debtor db)
        //{
        //    foreach (char sym in db.FullName)
        //        if (db.FullName.Count(s => s == sym) >= 3) return true;
        //    return false;
        //}).Sort((db, db2) => db.FullName.CompareTo(db2.FullName));

        #endregion

        #region 13)borclulardan en coxu hansi ilde dogulubsa hemin ili cixartmaq

        //List<int> years = debtors.Select(debtor => debtor.BirthDay.Year).ToList();
        //years.Sort((y, y2) => y.CompareTo(y2));
        //int mostYear = years[0];
        //foreach (int year in years)
        //    if (years.FindAll(y => y == mostYear).Count > years.FindAll(y => y == year).Count)
        //        mostYear = year;
        ...
        //List<int> debts = debtors.Select(debtor => debtor.Debt).ToList();
        //debts.Sort((d, d2) => d.CompareTo(d2));
        //debts.Reverse();
        //for (int i = 0; i < 5; i++)
        //    Console.WriteLine(debtors.First(db => db.Debt == debts[i]));
        ...
        //List<Debtor> filtered = debtors.FindAll((db) =>
        //{
        //    foreach (char sym in db.Phone)
        //        if (db.Phone.Count(s => s == sym) != 1 && sym != '-') return false;
        //    return true;
        //});

        //filtered.ForEach(db => Console.WriteLine(db));
        //Console.WriteLine($"Total Debt: {filtered.Sum(db => db.Debt)}");
        ...
        //debtors.FindAll(db =>
        //{
        //    int res = db.BirthDay.Month.CompareTo(DateTime.Today.Month);
        //    int month = DateTime.Today.Month - db.BirthDay.Month;
        //    if (res == 0)
        //        month = 1;
        //    else if (res < 0)
        //        month += 12;
        //    return db.Debt <= (month * 500);
        //})
        //    .ForEach(db => Console.WriteLine(db));
        ...
        debtors.FindAll(db =>
        {
            foreach (char sym in "smile")
                if (!db.FullName.ToLower().Contains(sym)) return false;
            return true;
